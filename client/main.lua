	...
	for k,v in pairs(Config.Menus) do
		...
		if data.value == k then
			...
			if v.modType then
				...
				if data.value == 'primaryRespray' or data.value == 'secondaryRespray' or data.value == 'pearlescentRespray' or data.value == 'modFrontWheelsColor' or data.value == 'modBackWheelsColor' then
					for i=1, #Config.Colors, 1 do
						...
						elseif data.value == 'modBackWheelsColor' then
							table.insert(elements, {label = Config.Colors[i].label, value = 'wheelColor', color = Config.Colors[i].value})
						end
					end
				else
